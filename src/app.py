...
# Widgets
x_axis = dcc.Dropdown(id="x_axis", options=[{'label': i, 'value': i} for i in ingredients], value="alcohol", clearable=False, style={"display": "inline-block", "width": "48%"})
y_axis = dcc.Dropdown(id="y_axis", options=[{'label': i, 'value': i} for i in ingredients], value="malic_acid", clearable=False, style={"display": "inline-block", "width": "48%"})
color_encode = dcc.Checklist(id="color_encode", options=[{'label': 'Color-Encode', 'value': 'Color-Encode'}])

multi_select = dcc.Dropdown(id="multi_select", options=[{'label': i, 'value': i} for i in ingredients], value=["alcohol", "malic_acid", "ash"], clearable=False, multi=True)
...
def update_scatter_chart(x_axis, y_axis, color_encode):
    return create_scatter_chart(x_axis, y_axis, bool(color_encode))
